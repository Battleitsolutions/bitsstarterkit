	...
	function ( $ ) {
		$.pluginInstall = {
			...
			'handleInstall': function () {
				var self = this;
				$( 'body' ).on( 'click', '.bits-install-plugin', function ( e ) {
					e.preventDefault();
					var button   = $( this );
					...
					wp.updates.installPlugin(
						{
							slug: slug,
							success: function () {
								button.text( bits_plugin_helper.activating + '...' );
...
jQuery( document ).ready( function ( $ ) {
	...
	$( '.btn-get-started' ).click( function ( e ) {
		...
		// Change button text.
		jQuery( this ).text( bits_redirect_demo_page.btn_text );
		...
		var data = {
			...
			security : bits_redirect_demo_page.nonce,
			state      : state,
		};
		...
		$.ajax({
			...
			success:function( response ) {
				extra_uri = '';
				if ( jQuery( '.bits-message-close' ).length ) {
					dismiss_nonce = jQuery( '.bits-message-close' ).attr( 'href' ).split( '_bits_notice_nonce=' )[1];
					extra_uri     = '&_bits_notice_nonce=' + dismiss_nonce;
				}
